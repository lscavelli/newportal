...
namespace App\Widgets\scavelli\contentlist;
...
class contentList extends Widget {
    ...
    public function getContent() {
        ...
        if ($this->config('feed')) {
            ...
            $this->conf['feedUrl'] = $this->request->url().'?'.$this->config('feed.feed_format')."&widgetid=".$this->config('id');
            $this->conf['feedJsonUrl'] = $this->request->url().'?json'."&widgetid=".$this->config('id');
        ...
        // controllare - sembra che esegua due volte la chiamata alla widget
    ...
    public function configWidget($widget) {
        return (new assetController($this->rp))->configWidget($widget, $this);
    }
