...
<fieldset>
    ...
    <div class="box-body" style="margin-right: 22px">
        ...
        <form method="POST" id="preferenceWidget" class="form-horizontal">
            ...
            {!! Form::hidden('modelWidgetId', $webContent->get('modelWidgetId'), ['id'=>"modelWidgetId"]) !!}
            {!! Form::hidden('modelWidget', $webContent->get('modelWidget'), ['id'=>"modelWidget"]) !!}
            ...
            <div class="form-group">
                <label for="model_widget_id" class="col-sm-2 control-label">Modello widget</label>
                <div class="col-sm-1">
                    {!! Form::text('model_widget_id', $webContent->get('modelwidgetsId'), ['class' => "form-control input-sm", 'id'=>"model_widget_id", 'disabled'=>'']) !!}
                </div>
                <div class="col-sm-8">
                    {!! Form::text('model_widget', $webContent->get('modelWidget'), ['class' => "form-control input-sm", 'id'=>"model_widget", 'disabled'=>'']) !!}
                </div>
                <div class="col-sm-1">
                    <a href="#" class="btn btn-danger btn-xs pull-right deleteModelWidget">Cancella</a>
                </div>
            </div>
        </form>
...
<fieldset style="margin-top: 5px!important;">
    ...
    <div class="box-body" style="margin-right: 22px">
        <form action="/admin/pages/{{ $webContent->get('widget')->pivot->page_id }}/configWidget/{{ $webContent->get('widget')->pivot->id }}" method="POST" id="selectStructure" class="form-horizontal">
            @csrf
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-1">
                    <a href="#" class="btn btn-warning btn-xs pull-right assignModelWidget">Aggiungi</a>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    $(".assignModelWidget").click(function() {
        $("#model_widget_id, #modelWidgetId").val($("#model_id").find('option:selected').val());
        $("#model_widget, #modelWidget").val($("#model_id").find('option:selected').text());
    });
    $(".deleteModelWidget").click(function() {
        var id = $(this).data('id');
        $('#model_widget_id, #model_widget, #modelWidget, #modelWidgetId').val('');
    });
